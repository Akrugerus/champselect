...
func UntarSingleFile(path string, r io.Reader) ([]byte, error) {
	...
	for {
		...
		// if the file is not the one we're searching for, continue
		case header.Name != path:
			continue
		}
		...
		// if its a dir it's invalid and we return nil
		case tar.TypeDir:
			return nil, nil

		// if it's a file it's valid
		case tar.TypeReg:
			// Create bytes buffer and buffer writer
			var b bytes.Buffer
			w := bufio.NewWriter(&b)
